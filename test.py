#!python3  ''' Create a program to iterate through the games and determine the win loss record  for each time  
# Create a dictionary for each team and store their number of wins, games played, goals for and goals against.  
# A dictionary template has been prepped for you to  use, although you can probably build one faster using iteration 
# or list comprehension 
# '''  
teams = ['AB','BC','MN','SK','ON','QC','PE','NS','NB','NL','YT','NT','NU']
games = [{'home': 'BC', 'homeScore': 0, 'away': 'YT', 'awayScore': 0}, {'home': 'NU', 'homeScore': 2, 'away': 'NS', 'awayScore': 2}, {'home': 'PE', 'homeScore': 0, 'away': 'YT', 'awayScore': 2},
...
teamData = {     'AB' : {         'gamesPlayed' : 0,         'wins' : 0,         'losses' : 0,         'ties' : 0,         'goalsFor' : 0,         'goalsAgainst' : 0     },     'BC' : {} } 
teamData = { i : {'gamesPlayed' : 0,'wins' : 0,'losses' : 0,'ties' : 0,'goalsFor' : 0,'goalsAgainst' : 0} for i in teams} #print(teamData) for  i in games: #Problem- Find the home team and assign the score to the dictionary inside the dictionary         home = games[games.index(i)]["home"]         teamData[home]["gamesPlayed"] += 1         if games[games.index(i)]["homeScore"] > games[games.index(i)]["awayScore"]:             teamData[home]["wins"] += 1         if games[games.index(i)]["homeScore"] < games[games.index(i)]["awayScore"]:             teamData[home]["losses"] += 1         if games[games.index(i)]["homeScore"] == games[games.index(i)]["awayScore"]:             teamData[home]["ties"] += 1         away = games[games.index(i)]["away"]         teamData[away]["gamesPlayed"] += 1         if games[games.index(i)]["awayScore"] > games[games.index(i)]["homeScore"]:             teamData[away]["losses"] += 1         if games[games.index(i)]["awayScore"] < games[games.index(i)]["homeScore"]:             teamData[away]["wins"] += 1         if games[games.index(i)]["awayScore"] == games[games.index(i)]["homeScore"]:             teamData[away]["ties"] += 1         teamData[home]["goalsFor"] += games[games.index(i)]["homeScore"]         teamData[home]["goalsAgainst"] += games[games.index(i)]["awayScore"] print(teamData)   def tests():     assert teamData['BC']['gamesPlayed'] == 12     assert teamData['BC']['wins'] == 5  tests()